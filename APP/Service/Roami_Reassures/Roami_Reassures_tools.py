...
def _nearby_search(keyword: str, lat: float, lng: float) -> list:
    """Google Places Nearby Search sorted by distance. Returns up to 5 nearest results."""
    try:
        params = {
            "keyword": keyword,
            "location": f"{lat},{lng}",
...
# Countries that officially use imperial/US customary units for road distances.
# Source: commonly cited list — US, Liberia, Myanmar use miles; everyone else uses km.
IMPERIAL_COUNTRY_CODES = {"US", "LR", "MM"}


def _get_units_for_coords(lat: float, lng: float) -> str:
    """
    Reverse-geocode lat/lng to determine the country, then return
    'imperial' or 'metric' based on that country's standard.
    Defaults to 'metric' if lookup fails.
    """
    try:
        response = httpx.get(
            "https://maps.googleapis.com/maps/api/geocode/json",
        ...
        if data.get("status") == "OK" and data.get("results"):
            for component in data["results"][0].get("address_components", []):
                if "country" in component.get("types", []):
                    ...
                    logger.info(f"[_get_units_for_coords] {lat},{lng} → country={country_code}")
                    return "imperial" if country_code in IMPERIAL_COUNTRY_CODES else "metric"
    except Exception as e:
        logger.error(f"[_get_units_for_coords] Failed for {lat},{lng}: {e}")
    return "metric"
...
def _get_distance(origin: str, destination: str) -> dict:
    ...
    Units (metric/imperial) are determined by reverse-geocoding the origin's
    GPS coordinates to find the country, then applying that country's standard.
    Falls back to text-based "USA" detection if origin has no GPS coords.
    """
    origin_lat, origin_lng = None, None
    geo_part = origin.split("|")[0] if "|" in origin else origin
    try:
        parts = geo_part.split(",")
        if len(parts) == 2:
            origin_lat, origin_lng = float(parts[0]), float(parts[1])
    except ValueError:
        pass

    if origin_lat is not None:
        units = _get_units_for_coords(origin_lat, origin_lng)
        is_usa = units == "imperial"
        origin = geo_part
    else:
        is_usa = any(
...
def nearby_place_search(keyword: str, origin: str) -> str:
    ...
    *** USE THIS TOOL when the user says "near me", "nearest", or "closest". ***
    ...
    PARAMETERS:
    - keyword: what to search for. E.g. "Pizzaburg", "coffee shop", "pharmacy"
    ...
    *** USE THIS TOOL whenever the response includes 2 or more places. ***
    ...
    async def _resolve_all():
        ...
        for pr in place_results:
            if pr.get("status") == "ok":
                place_lat = pr.get("lat")
                place_lng = pr.get("lng")
