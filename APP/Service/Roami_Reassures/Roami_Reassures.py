...
from ...prompt.prompt import Roami_Reassures_system_prompt,title_generation_prompt    
...
from .Roami_Reassures_tools import get_all_tools
...
class Roami_Reassures:
    ...
    async def _ensure_initialized(self):
        ...
        self.agent = create_agent(
            ...
            system_prompt=Roami_Reassures_system_prompt,
    ...
    async def _clear_checkpoint_completely(self, session_id: str) -> bool:
        """Clear corrupted checkpoint from MongoDB."""
        try:
            # Use sync client correctly — no await on sync MongoClient
            db = self.client.get_database(settings.DATABASE_NAME)

            # Log checkpoint state BEFORE clearing so you can debug the cause
            checkpoint_count = db["checkpoints"].count_documents({"thread_id": session_id})
            writes_count = db["checkpoint_writes"].count_documents({"thread_id": session_id})
            
            logger.warning(
                f"[checkpoint_clear] Corrupted checkpoint detected for session={session_id} | "
                f"checkpoints={checkpoint_count} | writes={writes_count}"
            )

            # Dump the writes so you can see which tool_call_id had no response
            if writes_count > 0:
                writes = list(db["checkpoint_writes"].find(
    ...
    async def get_response(self, session_id: str, user_input: str, user_mood: str, user_id: str):
            ...
            try:
                ...
                if final_text:
                    try:
                        ...
                        # Log but never surface to user — message was already delivered
                        logger.error(f"[get_response] save_travel_message failed: {save_err}")
                return
